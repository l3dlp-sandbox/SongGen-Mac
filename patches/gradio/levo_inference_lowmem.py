...
class LeVoInference(torch.nn.Module):
    ...
    def forward(self, lyric: str, description: str = None, prompt_audio_path: os.PathLike = None, genre: str = None, auto_prompt_path: os.PathLike = None, gen_type: str = "mixed", params = dict()):
        ...
        with torch.no_grad():
            # Handle 'separate' mode: generate all three outputs with the SAME tokens
            if gen_type == 'separate':
                if melody_is_wav:
                    wav_mixed = model.generate_audio(tokens, pmt_wav, vocal_wav, bgm_wav, gen_type='mixed', chunked=True)
                    wav_vocal = model.generate_audio(tokens, pmt_wav, vocal_wav, bgm_wav, gen_type='vocal', chunked=True)
                    wav_bgm = model.generate_audio(tokens, pmt_wav, vocal_wav, bgm_wav, gen_type='bgm', chunked=True)
                else:
                    wav_mixed = model.generate_audio(tokens, gen_type='mixed', chunked=True)
                    wav_vocal = model.generate_audio(tokens, gen_type='vocal', chunked=True)
                    wav_bgm = model.generate_audio(tokens, gen_type='bgm', chunked=True)
            else:
                if melody_is_wav:
        ...
        # Return dict for 'separate' mode, single tensor otherwise
        if gen_type == 'separate':
            return {'mixed': wav_mixed[0], 'vocal': wav_vocal[0], 'bgm': wav_bgm[0]}
